    ...
    return (
        <div className="w-full mx-auto p-4">
            <div className="flex mb-4">
                {asset.input ?
                    <InputCard input={asset.input} metadata={asset.metadata}/> :
            ...
            {isTraceOpen && 
                ...
                <RunsContextProvider id={asset.id}>
                    <DagRouterProvider>
                        <RunView runId={asset.id}/>
                    </DagRouterProvider>
                </RunsContextProvider>
                </div>
                }
            {asset.output ?
                <div className="flex mb-4">
                    <OutputCard output={asset.output} metadata={asset.metadata}/>
                </div> :
                <div className="w-[700px] bg-gray-200 text-gray-900 p-3 rounded-lg flex">
                    <Bug/> <span className="px-2">No Input</span>
                </div>}
        </div>
    )
