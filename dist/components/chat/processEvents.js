...
import { $insertBlock, $buildBlock, $buildChunk, $buildSent, $isPostfix, $insertPostfix, $insertBlockChunk, } from "./blockUtils";
export function useExecutionBuilder() {
    const stack = useRef([]); // span stack
    const roots = useRef([]); // root spans
    ...
    /** ─── Span Handling ─────────────────────────────── */
    function startSpan(streamEvent) {
    ...
    function pushSpanEvent(streamEvent) {
    ...
    function startStream(streamEvent) {
        const current = stack.current.at(-1);
        if (!current)
            return;
        ...
        const parentEvent = current.events.find((e) => e.id == streamEvent.parentEventId);
        if (!parentEvent) {
    ...
    function handleDelta(event) {
        ...
        const rootBlockEvent = current.events.find((e) => e.id === event.parentEventId);
        if (!rootBlockEvent)
            throw new Error("No root block event");
    ...
    /** ─── Dispatcher ─────────────────────────────────── */
    function processEvent(stream_event, streamHookId) {
