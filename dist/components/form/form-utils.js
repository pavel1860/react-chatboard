...
//     return true;
// }
export function fieldExistsInSchema(schema, fieldPath) {
    const pathSegments = fieldPath.split('.');
...
export function getZodFieldByPath(schema, path) {
    let current = schema;
...
export const useInputStyle = (props) => {
...
export const useFieldValues = (fieldPath) => {
    const { getFieldSchema: getFieldType } = useFormCtx();
    const enumValues = getFieldType(fieldPath).enum;
