...
const CtxProvider = ({ children }) => {
    ...
    const { branchId, conversationId, refUserId, setBranchId, setRefUserId, } = useStore();
    const urlParams = useCtxUrlParams();
    useEnsureGuestToken(status);
    useSyncUrlParams();
    return (_jsx(CtxContext.Provider, { value: {
            branchId: branchId,
...
const useAdminCtx = () => {
    const ctx = useHookCtx();
    const router = useRouter();
    const { setBranchId, setConversationId, setRefUserId, isUserListOpen, setIsUserListOpen, } = useStore();
    const setConversationIdAux = useCallback((partitionId) => {
        if (ctx.refUserId) {
...
const useCtx = () => {
    const ctx = useHookCtx();
    const router = useRouter();
    const { setBranchId, setConversationId, setRefUserId, } = useStore();
    const setConversationIdAux = useCallback((partitionId) => {
        setConversationId(partitionId);
...
//# sourceMappingURL=ctx-provider.js.map