...
import { useTurnBlocks } from "../services/turnService";
...
function findObjectIndexById(arr, id) {
...
function getItemsRelativeToIndex(arr, row, col, mode = 'after', includeSelf = false) {
    const result = [];
...
function buildRequest(message, addBranch, fromTurnId, files, ctx) {
...
const buildMessage = (message, isStreaming = false) => ({
    meta: {
...
function buildOptimisticTurn(history, content, state, toolCalls, fromTurnId, role) {
    const message = {
        content: content,
        choices: undefined,
        state: state,
...
function useSendMessage(history, mutate, ctx, setBranchId, onEvent, options = {
    ...
    const sendMessage = useCallback(async (content, toolCalls, state, fromTurnId, addBranch, files, role = "user") => {
        if (sending)
            return;
        setSending(true);
        try {
            ...
            const callStartStream = async (url, formData, userId) => {
                const res = await startStream({ url, formData, userId });
                return [...mockHistory];
            };
            await mutate(callStartStream(url, formData, session.user.auth_user_id.toString()), {
...
const useHistory = (partitionId, branchId, pendingMessage) => {
    const { events, pullEvents, consumed } = useStream();
    const optimisticDataRef = useRef(undefined);
    // const {
    //     data,
    //     items: turns,
    //     isLoading: loading,
    ...
    const { data, items: turns, isLoading: loading, error, mutate, setSize, size, } = useTurnBlocks({
        limit: 10,
    }, {
    ...
    useEffect(() => {
        ...
        if (newEvents.length === 0 || !dataRef.current)
            return;
...
function ChatProvider({ children }) {
    ...
    const { partitionId, setConversationId, branchId, setBranchId, } = useCtx();
    const { stateUpdateMessageId, setStateUpdateMessageId, pendingMessage, setPendingMessage, artifactView, setArtifactView, filters, setFilters, handleSearchQuery, setIsArtifactViewOpen, isArtifactViewOpen, } = useStore();
    ...
    //     limit: 10,
    // }, { keepPreviousData: !pendingMessage && !!partitionId, isDisabled: !partitionId })
    const { data, items: turns, isLoading: loading, error: messagesError, mutate: mutateMessages, setSize, size } = useHistory(partitionId, branchId, pendingMessage);
